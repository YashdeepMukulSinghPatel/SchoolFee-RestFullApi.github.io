...
module.exports.deleteQuestion = async (req, res) => {
    ...
    try {
        ...
        if (question) {
            ...
            // checking if any option has some Fee already
            ...
            for (let i = 0; i < options.length; i++) {
                if (options[i].Fee > 0) {
                    ...
                    return res.status(404).json({
                        message: "Question cannot be deleted, it's options are Fee Paid already !"
                    });
                }
            }

            // if no any Fee Submit on any option of question
